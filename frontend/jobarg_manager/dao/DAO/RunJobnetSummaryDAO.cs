...
namespace jp.co.ftf.jobcontroller.DAO
{
    ...
    public class RunJobnetSummaryDAO : BaseDAO
    {
        ...
        private String _select_run_jobnet_super = "select JR.* from "
                                        ...
                                        + "where (((JR.scheduled_time between ? and ?) OR (JR.start_time between ? and ?) OR ((JR.scheduled_time = 0) and (JR.start_time between ? and ?)) OR ((JR.scheduled_time = 0) and (JR.start_time = 0) and (multiple_start_up = 2))) or start_pending_flag = 1) AND delete_flag = 0 order by JR.scheduled_time,JR.start_time,JR.inner_jobnet_id";


        private String _select_run_jobnet = "SELECT JRAll.* FROM ("
                + "    ("
                + "        SELECT JR.*"
                + "        FROM ja_run_jobnet_summary_table AS JR"
                + "        WHERE ("
                + "            (JR.scheduled_time BETWEEN ? AND ?)"
                + "            OR (JR.start_time BETWEEN ? AND ?)"
                + "            OR (JR.scheduled_time = 0 AND JR.start_time BETWEEN ? AND ?)"
                + "            OR (JR.scheduled_time = 0 AND JR.start_time = 0 AND JR.multiple_start_up = 2)"
                + "        )"
                ...
                + "        SELECT JR.*"
                + "        FROM ja_run_jobnet_summary_table AS JR"
                + "        JOIN users AS U ON JR.user_name = U.username"
                + "        JOIN users_groups AS UG1 ON U.userid = UG1.userid"
                + "        JOIN users_groups AS UG2 ON UG1.usrgrpid = UG2.usrgrpid"
                + "        WHERE ("
                + "            (JR.scheduled_time BETWEEN ? AND ?)"
                + "            OR (JR.start_time BETWEEN ? AND ?)"
                + "            OR (JR.scheduled_time = 0 AND JR.start_time BETWEEN ? AND ?)"
                ...
                + "        AND JR.delete_flag = 0"
                + "        OR ("
                + "            JR.user_name = U.username"
                + "            AND U.userid = UG1.userid"
                + "            AND UG2.userid = ?"
                + "            AND JR.start_pending_flag = 1"
                + "            AND JR.delete_flag = 0"
                + "        )"
                + "    )"
                + "    UNION"
        ...
        private String _select_run_jobnet_not_belongGRP = "select JRAll.* from ((select JR.* from "
                                                      + "ja_run_jobnet_summary_table AS JR "
                                                      + "where (((JR.scheduled_time between ? and ?) OR (JR.start_time between ? and ?) OR ((JR.scheduled_time = 0) and (JR.start_time between ? and ?)) OR ((JR.scheduled_time = 0) and (JR.start_time = 0) and (multiple_start_up = 2))) and JR.public_flag=1 and JR.delete_flag=0)) "
                                                      + "union "
                                                      + "(select JR.* from "
                                                      ...
                                                      + "JR.public_flag=0 and JR.user_name=U.username and JR.delete_flag=0)) "
                                                      + " ) "
                                                      + "as JRAll "
                                                      + "order by JRAll.scheduled_time,JRAll.start_time,JRAll.inner_jobnet_id";
        ...
        private String _select_run_jobnet_err = "select JRAll.* from ((select JR.* "
                                        + "from ja_run_jobnet_summary_table AS JR where JR.job_status=2 and (JR.status=2 or JR.status=4 or JR.status=5) and JR.public_flag=1 and JR.delete_flag=0) "
                                        + "union "
                                        + "(select JR.* "
                                        ...
                                        + "JR.public_flag=0 and JR.user_name=U.username and U.userid=UG1.userid and UG2.userid=? and UG1.usrgrpid=UG2.usrgrpid and JR.delete_flag=0)) "
                                        + "as JRAll "
        ...
        private String _select_run_jobnet_err_super = "SELECT JR.* FROM ja_run_jobnet_summary_table AS JR "
                                            + "WHERE JR.job_status = 2 "
                                            + "AND (JR.status = 2 OR JR.status = 4 OR JR.status = 5) "
                                            + "AND JR.delete_flag = 0 "
                                            + "ORDER BY JR.start_time DESC, JR.inner_jobnet_id DESC";
