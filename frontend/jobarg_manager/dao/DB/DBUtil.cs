...
namespace jp.co.ftf.jobcontroller.DAO
{
    public class DBUtil
    {
        ...
        //OSS
        public static void UpdateCreatedDateColumn(DataTable table)
        {
            string columnName = "created_date";
            if (table.Columns.Contains(columnName))
            {
                DateTime now = GetSysTime();
                string createDate = now.ToString("yyyy-MM-dd HH:mm:ss");

                // Ensure all rows have a value for the 'created_date' column
                foreach (DataRow row in table.Rows)
                {
                    if (row.RowState == DataRowState.Added && (row[columnName] == DBNull.Value || row[columnName] == null))
                    {
                        row[columnName] = createDate;
                    }
                }
            }
        }
