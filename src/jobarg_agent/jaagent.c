 ...
 *                                                                            *
 ******************************************************************************/
int ja_agent_setenv(ja_job_object* job)
{
    int ret;
...
int ja_agent_begin(ja_job_object* job, char* current_time, char* datafile)
{
    ...
    case JA_AGENT_METHOD_TEST:
        ...
        if (strcmp(job->type, JA_PROTO_VALUE_REBOOT) == 0) {
            if( CONFIG_ALLOW_ROOT == 0) {
                zabbix_log(LOG_LEVEL_INFORMATION,"Reboot Debug: datafile : %s",datafile);
                zbx_snprintf(buff, strlen(datafile) - 4, "%s", datafile);
                removeFilePath(buff,&tmp_data_file);

                zbx_snprintf(filename, sizeof(filename), "%s-",tmp_data_file);

                zbx_snprintf(init_stat_file, sizeof(init_stat_file), "%s%c%s.job", JA_TEMP_FOLDER, JA_DLM, filename);
                zbx_snprintf(full_filepath, sizeof(full_filepath), "%s%c%s.job", JA_TEMP_FOLDER,JA_DLM,tmp_data_file);
                ...
                if (rename(full_filepath, init_stat_file) != 0) {
                    zabbix_log(LOG_LEVEL_ERR, "In %s(),[%s] job temp file cannot be renamed.%s", __function_name, tmp_data_file, strerror(errno));
                ...
                zbx_snprintf(full_filepath, sizeof(full_filepath), "%s%s", tmp_data_file, zero_pid);

                if (job_to_end(tmp_data_file, full_filepath,"temp") == FAIL) {
                    zabbix_log(LOG_LEVEL_ERR, "In %s() [%s*] files cannot be moved to end.", __function_name, filename);
                    goto error;
                }
                zbx_snprintf(job->message, sizeof(job->message), "Reboot job failed.[CONFIG_ALLOW_ROOT = %d]",CONFIG_ALLOW_ROOT);
                job->signal = 0;
    ...
    case JA_AGENT_METHOD_KILL:
        ...
        zbx_snprintf(jobid, sizeof(jobid), ZBX_FS_UI64"-", job->jobid);
        if(get_filename_by_jobid(tmp_exec_file, JA_EXEC_FOLDER, jobid) == SUCCEED) {
            ja_jobfile_getpid_by_filename(tmp_exec_file, temp_pid);
            if (temp_pid == NULL || temp_pid[0] == '\0' || atoi(temp_pid) == 0) {
                zabbix_log(LOG_LEVEL_INFORMATION, "In %s() pid for jobid cannot be read from status file.", __function_name);
            } else {
                job->pid = atoi(temp_pid);
                zabbix_log(LOG_LEVEL_DEBUG, "In %s(), file[%s] job->pid[%d] temp_pid[%s]", __function_name, tmp_exec_file, job->pid, temp_pid);   
            }
 ...
 *                                                                            *
 ******************************************************************************/
void ja_domain_check(char* full_username, char* user, char* domain , size_t user_size)
{
    char* backslash_ptr;
    const char* __function_name = "ja_domain_check";

    if (full_username == NULL || full_username[0] == '\0') {
        full_username = "";
    }

    backslash_ptr = strchr(full_username, '\\');
    zabbix_log(LOG_LEVEL_DEBUG, "In %s() Full user name : %s", __function_name,full_username);
    if (backslash_ptr != NULL) {
        zbx_strlcpy(user, backslash_ptr + 1, user_size);
        zbx_strlcpy(domain, full_username, backslash_ptr - full_username + 1); 
    } else {
        zbx_snprintf(user, user_size, "%s", full_username);
        domain[0] = '\0';
    }
    zabbix_log(LOG_LEVEL_DEBUG, "In %s() run user : %s", __function_name,user);
    zabbix_log(LOG_LEVEL_DEBUG, "In %s() domain : %s", __function_name,domain);
}
...
int ja_agent_run(ja_job_object* job, char* full_filename)
{
    ...
    char       w_domain[JA_MAX_STRING_LEN];
    char       cmd_user[JA_MAX_STRING_LEN];
    char       cmd_passwd[JA_MAX_STRING_LEN];
    char       cmd_domain[JA_MAX_STRING_LEN];
    ...
    if (ja_agent_setenv(job) == FAIL)
        goto error;
     if (job->method != JA_AGENT_METHOD_TEST)
    {
        zbx_snprintf(full_script, sizeof(full_script), "%s", job->script);
    }

    pid = getpid();
    zbx_snprintf(filepath, sizeof(filepath), "%s%c%s%c",JA_DATA_FOLDER, JA_DLM, data_filename, '-');
    zbx_snprintf(tmp_exe_file, sizeof(tmp_exe_file), "%s%c%s-", JA_EXEC_FOLDER, JA_DLM, data_filename);
    ...
    ja_domain_check(job->run_user, w_user, w_domain, sizeof(w_user));
    ...
    ja_domain_check(CONFIG_JA_COMMAND_USER, cmd_user, cmd_domain, sizeof(cmd_user));
    ...
    if ((strcmp(w_user, "") != 0) && w_user[0] != '\0' || (strcmp(w_domain, "") != 0) && w_domain[0] != '\0') {            /* run_user specified ? */
        isSeteuid = 0;
    }
    else if ((strcmp(cmd_user, "") != 0) && cmd_user[0] != '\0' || (strcmp(cmd_domain, "") != 0) && cmd_domain[0] != '\0') {   /* is command_user specified as jobarg_agentd.conf */
        zbx_snprintf(w_user, sizeof(w_user), "%s", cmd_user);
        zbx_snprintf(w_passwd, sizeof(w_passwd), "%s",cmd_passwd );
        zbx_snprintf(w_domain, sizeof(w_domain), "%s", cmd_domain);
        isSeteuid = 2;
    }
    else {
    ...
    if ((isSeteuid == 0) || (isSeteuid == 2)) {
        ...
        mbstowcs_s(&wLen, domain, sizeof(domain) / 2, w_domain, _TRUNCATE);
        ...
        if (!LogonUser(
            ...
            domain,                      /* A pointer to a null-terminated string that specifies the name of the domain or server               */
        ...
        zbx_snprintf(full_command, sizeof(full_command), "\"%s\" \"%s\" \"%s.%s\" \"%s\" \"%s\" \"%s\" \"%s\" \"%s\"", CONFIG_CMD_FILE, filepath, filepath, JA_EXE, w_user, w_passwd, tmp_exe_file, exec_file, JA_CHK_LOCK_FILE);
        wcommand = zbx_acp_to_unicode(full_command);
    }
    else {
        zbx_snprintf(full_command, sizeof(full_command), "\"%s\" \"%s\" \"%s.%s\" \"%s\" \"%s\" \"%s\" \"%s\" \"%s\"", CONFIG_CMD_FILE, filepath, filepath, JA_EXE, "", "", tmp_exe_file, exec_file, JA_CHK_LOCK_FILE);
        wcommand = zbx_acp_to_unicode(full_command);
    }
    ...
    if (0 == CreateProcess(NULL,        /* no module name (use command line) */
        ...
        // Clear the environment variables.
         for (entry = json_object_get_object(json_tokener_parse(job->env))->head;
             (entry ? (json_key = (char *)entry->k, val = (struct json_object *)entry->v, entry) : 0);
             entry = entry->next)
        {
            zbx_snprintf(env_var, sizeof(env_var), "%s=", json_key);
            putenv(env_var); 
        }
    }
    zbx_free(wcommand);
    ...
    if (pid == -1) {
        ...
        if (ja_agent_setenv(job) == SUCCEED) {
            ...
            execl(CONFIG_CMD_FILE, CONFIG_CMD_FILE, filepath, full_command, w_user, w_passwd, exefile, exec_file, JA_CHK_LOCK_FILE, NULL);
            zbx_snprintf(job->message, sizeof(job->message),
...
int write_json_data(ja_job_object* job, char* data_file) {
    ...
    */
    ...
    json_object_object_add(tmp_data, JA_PROTO_TAG_JOBPID, json_object_new_int(job->pid));
