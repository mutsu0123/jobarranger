...
static void jobarg_load_config(int requirement)
{
    struct cfg_line cfg[] = {
        ...
        {"ExtUnsignedFlag",       &CONFIG_EXT_UNSIGNED_FLAG,       TYPE_INT,    PARM_OPT,  0,   1},
        {NULL}
    };
    ...
    zbx_snprintf(JA_CHK_LOCK_FILE, sizeof(JA_CHK_LOCK_FILE), "%s%c%s", CONFIG_TMPDIR, JA_DLM, "chk_lock_file");
    //zbx_snprintf(JA_JOBS_FOLDER, sizeof(JA_JOBS_FOLDER), "%s%c%s", CONFIG_TMPDIR, JA_DLM, "abort"); ......why?
}
...
int MAIN_ZABBIX_ENTRY()
{
    ...
    zabbix_log(LOG_LEVEL_INFORMATION, "jobarg_agentd(): Current System ANSI code page :%u",codePage);
    ...
    // Check if the locale setting was successfully retrieved
    if (current_locale != NULL) {
        zabbix_log(LOG_LEVEL_INFORMATION, "jobarg_agentd(): Current LC_CTYPE :%s",current_locale);
    } else {
        zabbix_log(LOG_LEVEL_INFORMATION, "jobarg_agentd(): Current LC_TYPE is not set.");
    }
    ...
    // if (lc_ctype != NULL) {
    //     zabbix_log(LOG_LEVEL_INFORMATION, "jobarg_agentd(): Current LC_CTYPE :%s",lc_ctype);
    // } else {
    //     zabbix_log(LOG_LEVEL_INFORMATION, "jobarg_agentd(): Current LC_TYPE is not set.");
    // }

    // if (lang != NULL) {
    //     zabbix_log(LOG_LEVEL_INFORMATION, "jobarg_agentd(): Current LANG :%s",lang);
    // } else {
    //     zabbix_log(LOG_LEVEL_INFORMATION, "jobarg_agentd(): Current LANG is not set.");
    // }

    // Print the current locale
    //printf("Current locale: %s\n", current_locale);
    //zabbix_log(LOG_LEVEL_INFORMATION, "jobarg_agentd(): Current locale :%s", current_locale);
    ...
    // Test locking JA_CHK_LOCK_FILE.
    ja_unlock_resource(ja_lock_resource(JA_CHK_LOCK_FILE, JA_LOCK_SH | JA_LOCK_NB, "MAIN_ZABBIX_ENTRY"));
    ja_unlock_resource(ja_lock_resource(JA_CHK_LOCK_FILE, JA_LOCK_EX | JA_LOCK_NB, "MAIN_ZABBIX_ENTRY"));
