...
#endif
const char* RESOURCE_ERR = "Resource error: child process cannot access transaction files.";
/******************************************************************************
 *                                                                            *
 * Function:                                                                  *
 *                                                                            *
...
int ja_job_check_beginfile(ja_job_object* job){
	int ret = SUCCEED;
	int i, job_per_sec = 10, data_file_count = 0, begin_file_count = 0, proc_run_flag = 0, ext_cd = 137;
	int abort_flag = 0;
	...
	if(begin_file_count>0){
		...
		while(data_file_count<begin_file_count){
			...
			zbx_snprintf(tmp_filename, sizeof(filename), "%s",filename);
			
			getJobIdFromBeginFile(tmp_filename,18);
			job->jobid = atoi(tmp_filename);
			abort_flag = check_job_abort_flag(tmp_filename, 0);
			if (abort_flag ==  JA_ABORT_FILE_EXIST ){
				if(ja_abort_job_to_end(tmp_filename,filename,init_stat_file,JA_BEGIN_FOLDER) == SUCCEED){
					zabbix_log(LOG_LEVEL_INFORMATION,"In %s(), Abort flag file found job->jobid:"ZBX_FS_UI64", moved to end folder", __function_name, tmp_filename,job->jobid);
					goto contd;
				}
			}
			...
			if (proc_run_flag == 0) {
				...
				// Delete begin file 
				zabbix_log(LOG_LEVEL_INFORMATION, "In %s()  jobid: " ZBX_FS_UI64 " Job process is already running. process id : %d. delete [%s] file.", __function_name,job->jobid,job->pid,begin_file);
				if (FAIL != job_to_error(JA_BEGIN_FOLDER, filename))
					zabbix_log(LOG_LEVEL_WARNING, "[%s] ,along with the data files, are moved to error folder.", filename);
			}
...
void checkprocess_add_pid_to_filename(char* original, JA_PID pid){
	char tmp[JA_FILE_PATH_LEN];
	char buf[JA_FILE_PATH_LEN];
	char bufFullpath[JA_FILE_PATH_LEN];
	char originalFullpath[JA_FILE_PATH_LEN];
	const char* __function_name = "checkprocess_add_pid_to_filename";
	int i=0;
	int chk_lock_file_descriptor;
	zbx_snprintf(tmp,strlen(original)-3,"%s",original);
	zbx_snprintf(buf,JA_FILE_PATH_LEN,"%s%d",tmp,pid);
	while(FILE_EXT[i] != NULL){
		if(i==0){
			i++;
			continue;
		}
		zbx_snprintf(bufFullpath,JA_FILE_PATH_LEN,"%s%c%s.%s",JA_DATA_FOLDER,JA_DLM,buf,FILE_EXT[i]);
		zbx_snprintf(originalFullpath,JA_FILE_PATH_LEN,"%s%c%s.%s",JA_DATA_FOLDER,JA_DLM,tmp,FILE_EXT[i]);
		chk_lock_file_descriptor = ja_lock_resource(JA_CHK_LOCK_FILE, JA_LOCK_SH | JA_LOCK_NB, __function_name);
		if(rename(originalFullpath,bufFullpath) != 0){
			zabbix_log(LOG_LEVEL_WARNING,"In %s(), cannot rename old file :%s to new file:%s [%s]",__function_name,originalFullpath,bufFullpath,strerror(errno));
			ja_unlock_resource(chk_lock_file_descriptor);

			//revert the renamed files.
			while(i>0){
				zbx_snprintf(bufFullpath,JA_FILE_PATH_LEN,"%s%c%s.%s",JA_DATA_FOLDER,JA_DLM,buf,FILE_EXT[i]);
				zbx_snprintf(originalFullpath,JA_FILE_PATH_LEN,"%s%c%s.%s",JA_DATA_FOLDER,JA_DLM,tmp,FILE_EXT[i]);
	...
	zbx_snprintf(bufFullpath,JA_FILE_PATH_LEN,"%s%c%s.job",JA_EXEC_FOLDER,JA_DLM,buf);
	zbx_snprintf(originalFullpath,JA_FILE_PATH_LEN,"%s%c%s.job",JA_EXEC_FOLDER,JA_DLM,tmp);
	chk_lock_file_descriptor = ja_lock_resource(JA_CHK_LOCK_FILE, JA_LOCK_SH | JA_LOCK_NB, __function_name);
	rename(originalFullpath,bufFullpath);
	ja_unlock_resource(chk_lock_file_descriptor);
	zbx_snprintf(original,JA_FILE_PATH_LEN,"%s.job",buf);
}
//for E-3
	...
	if(file_count>0){
		...
		while(loop_count<file_count){
			...
			if(temp_pid == NULL || temp_pid[0] == '\0' || atoi(temp_pid) == 0){
				char *firstOccurrence = strchr(filename, '-');
				// If the first occurrence is found and there's another occurrence after it
				if (!(firstOccurrence && strchr(firstOccurrence + 1, '-'))) {
					goto contd;
				} 
			}
			ja_jobfile_fullpath(filename, JA_TYPE_STATUS_DATA, data_file);
			zabbix_log(LOG_LEVEL_DEBUG, "In %s() file full path [%s]", __function_name,data_file);
			if (read_datafile(job, data_file) == FAIL) {
			...
			if (GetExitCodeProcess(hProcess, &exitCode) != 0) {
				if (exitCode == STILL_ACTIVE) {
					...
					FILE *file;
					struct dirent *entry;
					char job_id[JA_FILE_PATH_LEN]; 
					char data_file_path[JA_FILE_PATH_LEN]; 
					char begin_file_path[JA_FILE_PATH_LEN]; 
					char exec_file_path[JA_FILE_PATH_LEN]; 
					char modifiedName[JA_FILE_PATH_LEN];	

					zabbix_log(LOG_LEVEL_INFORMATION,"One of the data files of job id : " ZBX_FS_UI64 " is used by another process. Reprocessing the job",job->jobid);
					zbx_snprintf(job_id, sizeof(job_id), "%d", job->jobid);
					...
					while ((entry = readdir(dir)) != NULL) {
						if (strstr(entry->d_name, job_id) != NULL && 
    						strcmp(entry->d_name + strlen(entry->d_name) - 5, ".json") != 0) {
							zbx_snprintf(data_file_path, sizeof(data_file_path), "%s%c%s", JA_DATA_FOLDER, JA_DLM, entry->d_name);
							if (remove(data_file_path) != 0) {
								zabbix_log(LOG_LEVEL_ERR,"In %s(), [%s] data file cannot be deleted.%s",__function_name, data_file_path, strerror(errno));
								closedir(dir);
								ret=FAIL;
								goto error;
							} 
							zabbix_log(LOG_LEVEL_DEBUG,"Deleted EBUSY data file : [%s]",data_file_path);
						}
					}					
					closedir(dir);
					...
					while ((entry = readdir(dir)) != NULL) {
						if (strstr(entry->d_name, job_id) != NULL) {							
							zbx_snprintf(exec_file_path, sizeof(exec_file_path), "%s%c%s", JA_EXEC_FOLDER, JA_DLM, entry->d_name);
							zbx_strlcpy(modifiedName, entry->d_name, sizeof(modifiedName));
							char *lastHyphen = strrchr(modifiedName, '-');  
							if (lastHyphen != NULL) {
								*lastHyphen = '\0';
							}
            				zbx_strlcat(modifiedName, ".job", sizeof(modifiedName) - strlen(modifiedName) - 1);
							zbx_snprintf(begin_file_path, sizeof(begin_file_path), "%s%c%s", JA_BEGIN_FOLDER, JA_DLM, modifiedName);
							
							file = fopen(exec_file_path, "w");
							if (file != NULL) {
								zabbix_log(LOG_LEVEL_DEBUG,"Cleared contents of: %s", exec_file_path);
								fclose(file);
								chk_lock_file_descriptor = ja_lock_resource(JA_CHK_LOCK_FILE, JA_LOCK_SH | JA_LOCK_NB, __function_name);
								if (rename(exec_file_path, begin_file_path) != 0) {
									zabbix_log(LOG_LEVEL_ERR,"In %s(), cannot move [%s] file to [%s].%s",__function_name, exec_file_path, begin_file_path, strerror(errno));
									ja_unlock_resource(chk_lock_file_descriptor);
									closedir(dir);
									ret=FAIL;
									goto error;
								...
								ja_unlock_resource(chk_lock_file_descriptor);
									...
									zabbix_log(LOG_LEVEL_ERR, "In %s, [%s] file cannot be opened.%s",__function_name, exec_file_path, strerror(errno));
									closedir(dir);
									ret=FAIL;
									goto error;
							}
					...
					zabbix_log(LOG_LEVEL_DEBUG, "In %s(),job id : " ZBX_FS_UI64 ", pid : %d - Process DEAD.", __function_name, job->jobid, child_pid);
					exitCode = fail_extcd;
				}
			}
			extcd = exitCode;
			...
			if(result_pid != 0)
			{
				...
				if (temp_pid == NULL || temp_pid[0] == '\0' || atoi(temp_pid) == 0) {
					zabbix_log(LOG_LEVEL_WARNING,"In %s(), job execution process exited, but process id is not updated on file. jobid: "ZBX_FS_UI64" pid:%d error code :%d",__function_name,job->jobid,job->pid,exitCode);
					//if there is no pid in the end of the filename, add pid to the filename.
					checkprocess_add_pid_to_filename(filename,job->pid);
					zbx_snprintf(job->message,sizeof(job->message),"%s job id : "ZBX_FS_UI64,RESOURCE_ERR,job->jobid);
			...
			} else if(check_job_abort_flag(job->jobid, 0) == JA_ABORT_FILE_EXIST){
				zabbix_log(LOG_LEVEL_INFORMATION,"In %s(), abort flag file found, kill jobid:"ZBX_FS_UI64, __function_name,job->jobid);
	...
	if(file_count>0){
		...
		while(loop_count<file_count){
			...
			if (file_size > 30) {
			    ...
			    if (chk == -1) {
		            job->result = JA_JOBRESULT_FAIL;
					if(strncmp(job->message,RESOURCE_ERR,strlen(RESOURCE_ERR)) != 0){
...
int ja_job_exec_close(ja_job_object* job){
	...
	if(file_count>0){
		...
		while(loop_count<file_count){
			...
			int temp_server_version_no = getProtoVersions(job->serverid);
			if (temp_server_version_no > job->version) {
				job->version = temp_server_version_no;
			} else {
				zabbix_log(LOG_LEVEL_INFORMATION, "In %s(), Using received protocol version: %f", __function_name, job->version);
			}
			...
			if(check_job_abort_flag(job->jobid, 0) == JA_ABORT_FILE_EXIST){
				job->signal = 1;
				job->method = JA_AGENT_METHOD_KILL;
			}
#ifdef _WINDOWS
			if(CONFIG_EXT_UNSIGNED_FLAG) {
				job->return_code = (unsigned long) job->return_code;
			}
#endif
