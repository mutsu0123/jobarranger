...
int ja_tcp_send_to(zbx_sock_t * s, ja_job_object * job, int timeout)
{
    ...
    if (ret == FAIL) {
        zabbix_log(LOG_LEVEL_DEBUG, "In %s() error: %s", __function_name,
                   zbx_tcp_strerror());
    }
    zbx_free(data);
    return ret;
...
int ja_tcp_recv_to(zbx_sock_t * s, ja_job_object * job, int timeout)
{
    ...
    */
    ...
    zbx_rtrim(data, "\r\n");
    //zabbix_log(LOG_LEVEL_INFORMATION,"-----------------------------utf8 data : %s",data);
    char * result;
    int res;
    ret = ja_telegram_from_server(data, job);
    // res = ja_utf8_to_locale(data,&result);
    // if(res == FAIL){
    //     zabbix_log(LOG_LEVEL_WARNING,"In %s(),utf8 to locale conversion error. Using utf8.",__function_name);
    //     ret = ja_telegram_from(data, job);
    // }else{
    //     zabbix_log(LOG_LEVEL_INFORMATION,"-----------------------------utf8 result : %s",result);
    //     ret = ja_telegram_from(result, job);
    // }
 ...
 *                                                                            *
 ******************************************************************************/
int ja_tcp_recv_to_server(zbx_sock_t * s, ja_job_object * job, int timeout)
{
    int ret, cnt=0;
    char *data;
    const char *__function_name = "ja_tcp_recv_to_server";
