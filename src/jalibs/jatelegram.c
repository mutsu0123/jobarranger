...
int ja_telegram_recv(ja_telegram_object * obj, zbx_sock_t * s, int timeout)
{
    ...
    if (strlen(data) == 0) {
        ...
        zabbix_log(LOG_LEVEL_DEBUG, "In %s() message: %s", __function_name, err);
        return ret;
   }
 ...
 *                                                                            *
 ******************************************************************************/
int ja_telegram_from_server(const char *telegram, ja_job_object * job)
{
    int ret;
    json_object *jp_telegram;
    const char *__function_name = "ja_telegram_from_server";
    ...
    ret = ja_telegram_from_server_head(jp_telegram, job);
    if (ret == FAIL)
        goto error;
 ...
 *                                                                            *
 ******************************************************************************/
int ja_telegram_from_server_head(json_object * json, ja_job_object * job)
{
    json_object *jp;
    char *telegram;
    double tele_version;
    const char *__function_name = "ja_telegram_from_server_head";
    ...
    if (job->version != tele_version && job->version != 2) {
        job->result = JA_RESPONSE_VERSION_FAIL;
        zbx_snprintf(job->message, sizeof(job->message),
                     "%s:[%d] does not match [%d]", JA_PROTO_TAG_VERSION,
                     job->version, tele_version);
...
int ja_telegram_to_jobresult(ja_job_object * job, json_object * json)
{
    ...
    char uint_return_code[24];

    zbx_snprintf(uint_return_code, sizeof(uint_return_code), "%lld", job->return_code);
    ...
    if (job->result == JA_JOBRESULT_SUCCEED) {
        ...
        json_object_object_add(jp_data, JA_PROTO_TAG_RET,
                               json_object_new_string(uint_return_code));
