...
int CONFIG_DB_CON_COUNT = 1;
int CONFIG_JAPURGE_LIMIT = 500;
char	*CONFIG_JAPURGE_TIME		= NULL;
...
static void zbx_set_defaults()
{
	...
	if(NULL == CONFIG_JAPURGE_TIME)
		CONFIG_JAPURGE_TIME = zbx_strdup(CONFIG_JAPURGE_TIME, "00:00-00:00");
}
...
//		
		...
		{"DBconCount",	&CONFIG_DB_CON_COUNT,	TYPE_INT, PARM_OPT,	1, 9999999},
		{"JaPurgeLimit",	&CONFIG_JAPURGE_LIMIT,	TYPE_INT, PARM_OPT,	1, 9999999},
		{"JaPurgeTime",		&CONFIG_JAPURGE_TIME,		TYPE_STRING,		PARM_OPT,	0,	0},
		{NULL}
	};
...
int ja_send_ipchange_request(pid_t **pid_list_ptr, int* temp_hosts_count){
	...
	pid_t * pid_list;
	int host_malloc = 125;
	char** hosts_list = (char**)malloc(host_malloc * sizeof(char*));
    ...
    while (NULL != (row1= DBfetch(result1))) {
		if(hosts_count > host_malloc) {
            host_malloc += 125;
            hosts_list = (char**)realloc(hosts_list, host_malloc * sizeof(char*));
		}
		hosts_list[hosts_count] = (char*)malloc(JA_HOST_NAME_LEN * sizeof(char));
		zbx_snprintf(hosts_list[hosts_count], JA_HOST_NAME_LEN,  "%s", row1[0]);
		hosts_count++;
	}
	DBfree_result(result1);
	while(NULL != (row2 = DBfetch(result2))) {
		is_duplicated = JA_FALSE;
		...
		for(int i = 0; i < hosts_count; i++) {
			if(strcmp(hosts_list[i], row2[0]) == 0) {
				is_duplicated = JA_TRUE;
				break;
			}
		}
		if(is_duplicated == JA_FALSE){
			hosts_list[hosts_count] = (char*)malloc(JA_HOST_NAME_LEN * sizeof(char));
			zbx_snprintf(hosts_list[hosts_count], JA_HOST_NAME_LEN, "%s", row2[0]);
			hosts_count++;
		}
	}
	*temp_hosts_count = hosts_count;
	...
	for(int i = 0; i < hosts_count; i++) {
    	...
    	hostid = ja_host_getip_ipchange(hosts_list[i], host_ip, NULL, JA_TXN_OFF);
		...
		while(1){
			...
			if(job_res.return_code == FAIL) {
				zabbix_log(LOG_LEVEL_INFORMATION, "In %s(), IP change request failed. Requested serverId: %s doesn't exist.", __function_name, job.serverid);
...
int MAIN_ZABBIX_ENTRY()
{
	...
	char jaz_folderpath[JA_FILE_PATH_LEN];
	...
	if(init_session_dbc_locks() != SUCCEED){
		zabbix_log(LOG_LEVEL_CRIT, "Can not init jobarg session locks.\nExiting jobarranger server.");
		exit(FAIL);
	}
