...
static int process_jarun()
{
	...
	if (NULL == (row2 = DBfetch(result2)))
	{
        ...
        case JA_JOB_METHOD_NORMAL:
            if (ja_joblog(JC_JOB_START, inner_jobnet_id, inner_job_id) == FAIL)
                zabbix_log(LOG_LEVEL_WARNING, "In %s() ja_joblog(JC_JOB_START) failed. inner_job_id: " ZBX_FS_UI64, __function_name, inner_job_id);
        ...
        case JA_JOB_METHOD_RERUN:
            if (ja_joblog(JC_JOB_RERUN, inner_jobnet_id, inner_job_id) == FAIL)
                zabbix_log(LOG_LEVEL_WARNING, "In %s() ja_joblog(JC_JOB_RERUN) failed. inner_job_id: " ZBX_FS_UI64, __function_name, inner_job_id);
            ret = jarun_normal(inner_job_id, job_type, test_flag);
            break;
        default:
