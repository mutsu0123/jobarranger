...
int jarun_icon_fcopy(const zbx_uint64_t inner_job_id, const int method)
{
    ...
    zabbix_log(LOG_LEVEL_DEBUG, "In %s() inner_job_id: " ZBX_FS_UI64,
               __function_name, inner_job_id, JA_TXN_OFF);
    ...
    result =
        ...
        ("select inner_job_id, inner_jobnet_id, from_host_flag, to_host_flag, overwrite_flag,"
         ...
         ZBX_FS_UI64, inner_job_id, JA_TXN_OFF);
    if (NULL != (row = DBfetch(result))) {
        ...
        if (ja_cpy_value(inner_job_id, row[6], from_directory) == FAIL) {
            ja_log("JARUNICONFCOPY200004", 0, NULL, inner_job_id,
                   __function_name, row[6], inner_job_id, JA_TXN_OFF);
        ...
        if (ja_cpy_value(inner_job_id, row[7], from_file_name) == FAIL) {
            ja_log("JARUNICONFCOPY200004", 0, NULL, inner_job_id,
                   __function_name, row[7], inner_job_id, JA_TXN_OFF);
            DBfree_result(result);
            return ja_set_runerr(inner_job_id, 2);
        }
        ...
        if (ja_cpy_value(inner_job_id, row[9], to_directory) == FAIL) {
            ja_log("JARUNICONFCOPY200004", 0, NULL, inner_job_id,
                   __function_name, row[9], inner_job_id, JA_TXN_OFF);
            DBfree_result(result);
            return ja_set_runerr(inner_job_id, 2);
        }
    } else {
    ...
    if (ja_host_getname
        ...
        ja_log("JARUNICONFCOPY200005", 0, NULL, inner_job_id,
               __function_name, from_host_name, from_host_flag, inner_job_id, JA_TXN_OFF);
        return ja_set_runerr(inner_job_id, 2);
    }
    ...
    if (ja_host_getname(inner_job_id, to_host_flag, to_host_name, to_host)
        ...
        ja_log("JARUNICONFCOPY200006", 0, NULL, inner_job_id,
               __function_name, to_host_name, to_host_flag, inner_job_id, JA_TXN_OFF);
        return ja_set_runerr(inner_job_id, 2);
    }
    ...
    if (ja_host_lockinfo(from_host) == SUCCEED) {
        zabbix_log(LOG_LEVEL_DEBUG,
                   ...
                   ZBX_FS_UI64, __function_name, from_host, inner_job_id, JA_TXN_OFF);
        return FAIL;
    }
    if (ja_host_lockinfo(to_host) == SUCCEED) {
        zabbix_log(LOG_LEVEL_INFORMATION,
                   "In %s() host '%s' is locked. inner_job_id: "
                   ZBX_FS_UI64, __function_name, to_host, inner_job_id, JA_TXN_OFF);
        return FAIL;
    }
    ...
    if (ja_connect_to_port(&sock_to, to_host, inner_job_id, JA_TXN_OFF) == FAIL) {
        msg_skip = 1;
        DBclose();
        goto error;
    }
    ...
    DBconnect(ZBX_DB_CONNECT_ONCE);
    if (ja_connect_to_port(&sock_from, from_host, inner_job_id, JA_TXN_OFF) == FAIL) {
        msg_skip = 1;
        DBclose();
        goto error;
    }
  ...
  error:
    ...
    if (ret == SUCCEED) {
        if (ja_flow(inner_job_id, JA_FLOW_TYPE_NORMAL, 1) == FAIL) {
            zabbix_log(LOG_LEVEL_ERR, "In %s() ja_flow() failed. inner_job_id: " ZBX_FS_UI64, __function_name, inner_job_id, JA_TXN_OFF);
        }
    } else {
        if (strlen(errmsg) == 0) {
        ...
        if (msg_skip == 0) {
            ja_log("JARUNICONFCOPY200003", 0, NULL, inner_job_id, __function_name, errmsg, inner_job_id, JA_TXN_OFF);
        }
        if (ja_set_runerr(inner_job_id, 2) == FAIL) {
            zabbix_log(LOG_LEVEL_ERR, "In %s() ja_set_runerr() failed. inner_job_id: " ZBX_FS_UI64, __function_name, inner_job_id, JA_TXN_OFF);
        }
    }
    DBclose();
