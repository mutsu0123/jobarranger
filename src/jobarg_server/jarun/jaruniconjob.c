...
int jarun_icon_job_getenv(zbx_uint64_t inner_job_id, ja_job_object * job)
{
    ...
    zabbix_log(LOG_LEVEL_DEBUG, "In %s() inner_job_id: " ZBX_FS_UI64, __function_name, inner_job_id, JA_TXN_OFF);

    jp_env = json_object_new_object();
    result = DBselect("select value_name from ja_run_value_jobcon_table"
                      " where inner_job_id = " ZBX_FS_UI64, inner_job_id, JA_TXN_OFF);
...
int jarun_icon_job(zbx_uint64_t inner_job_id, int flag)
{
    ...
    if (row != NULL) {
        ...
        ja_log("JARUNICONJOB200017", inner_jobnet_id, NULL, inner_job_id, inner_job_id, JA_TXN_OFF);
        DBfree_result(result);
    ...
    if (ja_host_getname(inner_job_id, host_flag, host_name, host) == FAIL) {
        ja_log("JARUNICONJOB200028", inner_jobnet_id, NULL, inner_job_id, host_name, host_flag, inner_job_id, JA_TXN_OFF);
        return ja_set_runerr(inner_job_id, 2);
    }
    ...
    if (flag == JA_AGENT_METHOD_NORMAL || flag == JA_AGENT_METHOD_TEST) {
        if (ja_host_lockinfo(host) == SUCCEED) {
            zabbix_log(LOG_LEVEL_DEBUG,
                       ...
                       ZBX_FS_UI64, __function_name, host, inner_job_id, JA_TXN_OFF);
            return FAIL;
        }
    }
    ...
    if (flag == JA_AGENT_METHOD_ABORT) {
        ...
        row = DBfetch(result);
        if (row == NULL) {
            ja_log("JARUNICONJOB200024", inner_jobnet_id, NULL, inner_job_id, inner_job_id, JA_TXN_OFF);
            DBfree_result(result);
            return ja_set_runerr(inner_job_id, 2);
        }
    ...
    pid = ja_fork();
    if (pid == -1) {
        ja_log("JARUNICONJOB200027", inner_jobnet_id, NULL, inner_job_id, inner_job_id, JA_TXN_OFF);
        return ja_set_runerr(inner_job_id, 2);
    } else if (pid != 0) {
    ...
    if (ja_connect_to_port(&sock, host, inner_job_id, JA_TXN_OFF) == FAIL) {
