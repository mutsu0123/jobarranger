...
int ja_lock_resource_jobarg_cmd(const char *lock_file_path, int lock_mode)
{
    const char *__function_name = "ja_lock_resource_jobarg_cmd";
    ...
    while (1)
    {
        ...
        {
            fprintf(stderr, "Unable to open lock file: %s. Error: %s", lock_file_path, strerror(errno));
            exit(-1);
        }
    ...
    while (1)
    {
        file_descriptor = open(lock_file_path, O_RDWR);
...
void ja_unlock_resource_jobarg_cmd(int file_descriptor)
{
    const char *__function_name = "ja_unlock_resource_jobarg_cmd";
    ...
    while(1) {
        ...
        {
            fprintf(stderr, "Unable to unlock the file, Error: %s", strerror(errno));
            Sleep(5000);
        }
    }
#else
...
{
    char * orig_exe_file, * tmp_exe_file, * tmp_filepath, * tmp_cmd, * JA_CHK_LOCK_FILE;
    char filepath[SIZE];
    char exe_file[SIZE];
    char cmd[SIZE];
    char fn[SIZE];
    ...
    char fn_end[SIZE];
    int i;
    int fd, fd_start, fd_end, fd_ret, fd_stdout, fd_stderr, chk_lock_file_descriptor;
    time_t now;
    ...
    chk_lock_file_descriptor = ja_lock_resource_jobarg_cmd(JA_CHK_LOCK_FILE, JA_LOCK_SH | JA_LOCK_NB);
     if(rename(old_cmd_name, new_cmd_name) != 0) {
        ja_unlock_resource_jobarg_cmd(chk_lock_file_descriptor);
