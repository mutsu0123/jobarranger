...
-- Job Arranger upgrade table SQL for MySQL (Ver 6.0.8)  - 2024/08/31 -
...
DROP PROCEDURE IF EXISTS AddDeleteFlagColumnIfNotExists;
...
CREATE PROCEDURE AddDeleteFlagColumnIfNotExists()
BEGIN
    ...
    WHERE TABLE_SCHEMA = DATABASE() 
      AND TABLE_NAME = 'ja_run_jobnet_summary_table'
      AND COLUMN_NAME = 'delete_flag';

    -- If the column does not exist, add it
    IF col_exists = 0 THEN
        ALTER TABLE ja_run_jobnet_summary_table ADD COLUMN delete_flag INT DEFAULT 0 NULL;
    END IF;
END 
//
DELIMITER ;
...
CALL CreateIndexIfNotExists('ja_run_flow_table', 'ja_run_flow_idx1', 'start_inner_job_id');
...
CALL AddDeleteFlagColumnIfNotExists();